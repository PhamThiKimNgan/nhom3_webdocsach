...
export const AdminController = {
   ...
   updateRoles:async(req,res)=>{
        try{
            ...
            if(id){
                const newUser=await User.updateOne({id},{roles:roles.map(item=>item.id)},{new:true})
                if(newUser){
                ...
                return res.status(400).json(ResponseDetail(400,{message:"Không có id"}))
        }
        catch (error) {
            console.log(error)
            return res.status(500).json(ResponseDetail(500,{message:"Lỗi cập nhật quyền tài khoản"}))
    ...
    GetUserById: async (req,res)=>{
        try {
            const userId = req.params.userId; // Changed from req.body.userId to get ID from URL parameter
    ...
    SearchUsers: async (req, res) => {
        try {
            ...
            if (!searchQuery) {
                return res.status(400).json(ResponseDetail(400, { message: "Cần cung cấp từ khóa tìm kiếm" }));
            }
            ...
            return res.status(200).json(ResponseData(200, users));
        } catch (error) {
            console.log(error);
            return res.status(500).json(ResponseDetail(500, { message: "Lỗi tìm kiếm người dùng" }));
        }
    }
}