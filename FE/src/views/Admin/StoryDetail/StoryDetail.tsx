...
import { getNovelReviewById, getChaptersByNovelId, deleteNovelById } from '../../../api/apiStory';
import { getListCommentByNovelId, deleteComment } from '../../../api/apiComment';
...
const StoryDetail = () => {
  ...
  const [deleteType, setDeleteType] = useState<'comment' | 'rating' | 'story'>('comment');
  ...
  const fetchNovelData = async () => {
    ...
    try {
      ...
      const [novelData, chaptersData, commentsData, ratingsData] = await Promise.all([
    ...
    try {
      ...
      if (novel) {
        setNovel({
          ...
          ratingAvg: updatedRatings.length > 0 
            ? updatedRatings.reduce((acc, curr) => acc + curr.score, 0) / updatedRatings.length 
            : 0
        });
      }
    } catch (error) {
  ...
  const openDeleteModal = (type: 'comment' | 'rating' | 'story', item?: Comment | Rating) => {
    setDeleteType(type);
        ...
        <div className="delete-confirmation">
          ...
          <p>
            ...
            <span className="text-danger">
              Hành động này sẽ xóa tất cả các chương, bình luận và đánh giá liên quan.
