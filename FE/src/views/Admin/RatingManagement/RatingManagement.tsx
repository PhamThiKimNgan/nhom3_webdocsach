    ...
    try {
      ...
      const processedData = data.map(rating => ({
        ...
        username: typeof rating.username === 'string' ? rating.username : 
                 (rating.userId && typeof rating.userId === 'object' && rating.userId.username) || 'Unknown User',
        novelId: typeof rating.novelId === 'string' ? rating.novelId : '',
        novelName: typeof rating.novelName === 'string' ? rating.novelName : 
                  (rating.novelId && typeof rating.novelId === 'object' && rating.novelId.name) || 'Unknown Novel',
          ...
          {currentRatings.length > 0 ? (
            currentRatings.map((rating) => (
              <tr key={rating.id}>
                ...
                <td>{rating.novelName || rating.novelId}</td>
          ...
          <ModalContent onClose={closeDeleteModal}>
            <div className="delete-confirmation">
              ...
              <div className="content-preview">
                <strong>Đánh giá:</strong> {renderStars(selectedRating?.score || 0)}
